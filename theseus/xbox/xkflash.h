// xkflash.h: LPC bus flash protocol primitives. AMD-compatible flash
// command sequences (program / erase / unlock) used by the modchip
// detection path in xbox/modchip.cpp.
...
//Xenium Goodness
#define XENIUM_MANUF_ID1 0x01
#define XENIUM_MANUF_ID2 0xC2
...
// Special case: Xenium CPLD often responds with fixed ID or forwards CPLD flash mapping
if ((manuf == XENIUM_MANUF_ID1 || manuf == XENIUM_MANUF_ID2) && code == XENIUM_DEVICE_ID)
{
	static fci_t xenium_fci = {
		XENIUM_MANUF_ID1, XENIUM_DEVICE_ID,
		"XENIUM (CPLD Detected)",
		0x200000, // 2MB
		NULL
	};
	return &xenium_fci;
